...
Transport.prototype.onMessage = function (packet) {
  ...
  if ('heartbeat' == packet.type) {
    ...
    if (current) {
      this.manager.log.debug('handle packet by itself', this.id, packet);
      this.manager.onClientMessage(this.id, packet);
    } else {
      this.manager.log.debug('cant handle, pub packet', this.id, packet);
...
Transport.prototype.onClose = function () {
  this.manager.log.debug('transport on close:', this.id);
  if (this.open) {
    this.setCloseTimeout();
    this.clearHandlers();
    this.open = false;
    this.manager.onClose(this.id);
    this.manager.log.debug('pub close', this.id);
