...
HTTPPolling.prototype.handleRequest = function (req) {
  ...
  if (req.method == 'GET') {
    ...
    this.pollTimeout = setTimeout(function () {
      ...
      self.log.debug(self.name + ' closed due to exceeded duration', self.id);
...
HTTPPolling.prototype.clearPollTimeout = function () {
  if (this.pollTimeout) {
    ...
    this.log.debug('clearing poll timeout: ', this.id);
  }
...
HTTPPolling.prototype.write = function (data, close) {
  this.manager.log.debug('write data and end response: ', this.id, data);
  this.doWrite(data);
  this.response.end();
  this.onClose();
};
