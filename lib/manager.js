...
Manager.prototype.initStore = function () {
  ...
  this.store.subscribe('handshake', function (id, data) {
    self.log.debug('get handshake from redis', id);
    self.onHandshake(id, data);
  });

  this.store.subscribe('connect', function (id) {
    self.log.debug('get connect from redis: ', id);
    self.onConnect(id);
  });

  this.store.subscribe('open', function (id) {
    self.log.debug('get open from redis: ', id);
    self.onOpen(id);
  });
  ...
  this.store.subscribe('close', function (id) {
    self.log.debug('get close from redis:', id);
    self.log.debug('emit onClose');
    self.onClose(id);
  });
  ...
  this.store.subscribe('disconnect', function (id) {
    self.log.debug('get disconnect from redis:', id);
...
Manager.prototype.onHandshake = function (id, data) {
  this.log.debug('add handshake: ', id);
...
Manager.prototype.onOpen = function (id) {
  ...
  if (this.closed[id]) {
    ...
    if (self.closed[id] && self.closed[id].length && transport) {
      ...
      if (transport.open) {
        self.log.debug('transport payload closed and clear closed:', id, self.closed[id]);
...
Manager.prototype.onClientDispatch = function (id, packet) {
  if (this.closed[id]) {
    this.log.debug('on client dispatch, push packet to closed', id, packet, this.closed[id]);
...
Manager.prototype.onClientMessage = function (id, packet) {
  if (this.namespaces[packet.endpoint]) {
    this.log.debug('handle client message', id, packet);
...
Manager.prototype.onDisconnect = function (id) {
  ...
  this.log.debug('on disconenct, clear everything for ', id);
  if (this.open[id]) {
    delete this.open[id];
  }
  ...
  if (handshaken) {
    if (transport.open) {
      if (this.closed[data.id] && this.closed[data.id].length) {
        this.log.debug('payload closed and clear closed', data.id, this.closed[data.id]);
    ...
    if (transport.open) {
      ...
      this.log.debug('client not handshaken', data.id);
    }
...
Manager.prototype.handleHandshake = function (data, req, res) {
  ...
  this.authorize(handshakeData, function (err, authorized, newData) {
    ...
    if (authorized) {
      ...
      self.log.debug('pub handshake', id);
      self.store.publish('handshake', id, newData || handshakeData);

      var delay = self.get('polling response delay');
      if (delay) {
        setTimeout(function() {
          res.end(hs);
        }, delay);
