...
function Manager (server, options) {
  ...
  server.once('listening', function () {
    self.gc = setInterval(self.garbageCollection.bind(self), self.get('gc interval') * 1000);
  });
...
Manager.prototype.initStore = function () {
  ...
  this.store.subscribe('handshake', function (id, data) {
    self.log.debug('handshake from redis:', id);
    self.onHandshake(id, data);
  });

  this.store.subscribe('connect', function (id) {
    self.log.debug('connect from redis:', id);
    self.onConnect(id);
  });

  this.store.subscribe('open', function (id) {
    self.log.debug('open from redis:', id);
    self.onOpen(id);
  });
...
Manager.prototype.onHandshake = function (id, data) {
  this.log.debug('on handshake: ', id);
...
Manager.prototype.onOpen = function (id) {
  this.log.debug('on open:', id);
  this.open[id] = true;

  if (this.closed[id]) {
    var self = this;

    this.log.debug('unsubscribe dispatch: ', id);
    this.store.unsubscribe('dispatch:' + id, function () {
      ...
      if (self.closed[id] && self.closed[id].length && transport) {
        ...
        if (transport.open) {
          self.log.debug('on open payload buffer & clear buffer cached:', id, self.closed[id]);
    ...
    for (var i = 0, l = this.rooms[room].length; i < l; i++) {
      ...
      if (!~exceptions.indexOf(id)) {
        if (this.transports[id] && this.transports[id].open) {
          this.log.debug('send directly', id);
          this.transports[id].onDispatch(packet, volatile);
        } else if (!volatile) {
          this.log.debug('push buffer to closed', id);
...
Manager.prototype.onClose = function (id) {
  this.log.debug('on close: ', id);
  if (this.open[id]) {
    delete this.open[id];
  }

  this.log.debug('on close clear closed: ', id, this.closed[id]);
  this.closed[id] = [];

  var self = this;

  this.log.debug('subscribe dispatch:', id);
  this.store.subscribe('dispatch:' + id, function (packet, volatile) {
    if (!volatile) {
      self.log.debug('push packet to closed', id, packet);
...
Manager.prototype.onClientDispatch = function (id, packet) {
  if (this.closed[id]) {
    this.log.debug('add packet to closed', id, packet);
...
Manager.prototype.onClientDisconnect = function (id, reason, local) {
  this.log.debug('on client disconnect: ', id);
...
Manager.prototype.onDisconnect = function (id, local) {
  this.log.debug('on disconnect: ', id, local);
  this.log.debug('clear all the info about', id);
  ...
  if (this.closed[id]) {
    this.log.debug('disconnect clear cache:', id);
  ...
  if (handshaken) {
    if (transport.open) {
      if (this.closed[data.id] && this.closed[data.id].length) {
        ...
        this.log.debug('handle client send buffer & clear closed', data.id, this.closed[data.id]);
        this.closed[data.id] = [];
      }

      this.log.debug('emit onOpen', data.id);
      this.onOpen(data.id);
      this.log.debug('pub open: ', data.id);
    ...
    if (!this.connected[data.id]) {
      ...
      this.log.debug('pub connect: ', data.id);
      this.store.publish('connect', data.id);

      // flag as used
      this.log.debug('chear handshake issued and re-pub:', data.id);
      ...
      this.store.subscribe('message:' + data.id, function (packet) {
        self.log.debug('get client message from redis:', data.id, packet);
        self.onClientMessage(data.id, packet);
      });

      this.store.subscribe('disconnect:' + data.id, function (reason) {
        self.log.debug('get disconnect from redis', data.id);
        self.onClientDisconnect(data.id, reason);
      });
    }
  } else {
    if (transport.open) {
      self.log.debug('client not handshake:', data.id);
...
Manager.prototype.handleHandshake = function (data, req, res) {
  ...
  this.authorize(handshakeData, function (err, authorized, newData) {
    ...
    if (authorized) {
      ...
      self.log.debug('handshake agent: ', id, agent);
...
Manager.prototype.garbageCollection = function () {
  ...
  // this.log.debug('gc start');
  while (i--) {
    handshake = this.handshaken[ids[i]];

    if ('issued' in handshake && (now - handshake.issued) >= this.get('handshake expiration')) {
     // this.log.debug('gc handshake: ', ids[i]);
      this.onDisconnect(ids[i]);
    }
  }
  // this.log.debug('gc end');
  // this.log.debug('gc duration: ', Date.now() - now);
};
