  ...
  if (this.closed[id]) {
    ...
    this.store.unsubscribe('dispatch:' + id, function () {
      ...
      if (self.closed[id] && self.closed[id].length && transport) {
        ...
        if (transport.open) {
          self.log.debug('on open clear buffer cached:', id);
...
Manager.prototype.onClose = function (id) {
  this.log.debug('manager on close: ', id);
  if (this.open[id]) {
    delete this.open[id];
  }

  this.log.debug('on close clear cached msg: ', id);
  this.closed[id] = [];
...
Manager.prototype.onClientDispatch = function (id, packet) {
  if (this.closed[id]) {
    this.log.debug('add cached buffer', id, packet);
  ...
  if (handshaken) {
    if (transport.open) {
      if (this.closed[data.id] && this.closed[data.id].length) {
        ...
        this.log.debug('handle client clear buffer', data.id);
        this.closed[data.id] = [];
      }
