...
Manager.prototype.handleUpgrade = function (req, socket, upgradHead) {
  var data = this.checkRequest(req);
...
Manager.prototype.onWebsocketConnection = function (data, wsSocket) {
  ...
  var socket = this.createOrGetSocket(self, data.id, 'Dummy handshaken data for websocket');
...
Manager.prototype.handleHTTPRequest = function (data, req, res) {
  ...
  // it must jsonp here
  Trans = transports[data.transport];
    ...
    if (handshaken) {
      ...
      var writeErr = function (message) {
        if (data.transport === 'jsonp-polling') {
          ...
          req.res.writeHead(200, { 'Content-Type': 'application/javascript' });
          req.res.end('io.j[' + i + ']("' + message + '");');
        } else {
